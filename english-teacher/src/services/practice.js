...
export const practiceService = {
  ...
  getPractices: async ({ lessonId }) => {
    try {
      ...
      let formatDataRes = null;
      if (response.data?.data) {
        const callApiPartsExam = response?.data?.data?.partResponseDTOS?.map(
          (partExam) => API.get(`/api/v1/part/${partExam?.id}`)
        );
        const resPartsExam = await axios.all(callApiPartsExam);
        formatDataRes = resPartsExam.reduce(
  ...
  getPracticesByExam: async ({ examId }) => {
    try {
      const response = await API.get(`/api/v1/exam/${examId}`);
      const callApiPartsExam = response?.data?.data?.partResponseDTOS?.map(
        (partExam) => API.get(`/api/v1/part/${partExam?.id}`)
      );
      const resPartsExam = await axios.all(callApiPartsExam);
  ...
  getPractice: async (practiceId) => {
    try {
      const response = await API.get(`/api/v1/partExam/${practiceId}`);
      return response;
    } catch (error) {}
  },
  ...
  deletePractice: async ({ practiceId }) => {
    try {
      const response = await API.delete(`/api/v1/exam?examId=${practiceId}`);
      return response;
    } catch (error) {}
  },
};
