...
export const teacherService = {
  ...
  updateTeacher: async ({ teacherId, updateData }) => {
    try {
      const response = await API.put(`/api/v1/user/${teacherId}`, updateData);
      return response;
    } catch (error) {}
  },
...
export const userCommonService = {
  ...
  changePassword: async ({ userId, updateData }) => {
    try {
      const response = await API.put(`/api/v1/user/password/${userId}`, updateData);
      return response
    } catch (error) {}
  },
...
export const studentService = {
  ...
  getPracticesByStudent: async ({ userId }) => {
    try {
      const response = await API.get(
        `/api/v1/practice/result/user/${userId}?size=10&page=1`
      );
