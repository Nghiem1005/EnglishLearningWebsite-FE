...
const EditCourse = () => {
  ...
  const { isLoading: isLoadingTeacher, data: dataTeacher } = useQueryCustom(
  ...
  const onSubmit = async (form) => {
    try {
      const { level, teacherId, price, name } = form;
  ...
  if (isLoadingTeacher || isLoadingCourse) return;
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                        ...
                        <select
                          ...
                          defaultChecked={dataCourse?.data?.data?.teacher?.id}
                        >
