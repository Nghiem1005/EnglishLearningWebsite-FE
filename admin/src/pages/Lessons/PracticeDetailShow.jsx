...
const PracticeDetailShow = ({ data, setIsShowDetail }) => {
  ...
  const handleDeletePractice = async ({ practiceId }) => {
    const { data } = await practiceService.deletePractice({ practiceId });
    if (data.status === "OK") {
      ...
      <div className="top-[20%] overflow-y-auto p-6 md:p-10 z-40 bg-white rounded-tl-3xl rounded-tr-xl absolute bottom-0 left-0 right-0 opacity-100">
        ...
        {dataPractice ? (
              ...
              <>
                <div>
                  <h3 className="font-bold">
                    Phần {partPractice?.serial}: {partPractice?.description}
                  </h3>
                  {partPractice?.questionPhraseResponseDTOS?.map(
                    (partQuestion) => (
                      <>
                        <h2>Đề {partQuestion?.serial}:</h2>
                        ...
                        {partQuestion?.questionResponseDTOS?.map(
                          (question, index) => (
                            <div key={index} className=" bg-gray-50 p-4 mb-10 mt-1 mx-3 rounded-sm">
                              ...
                              {question.answerResponseDTOS.map(
                                (answer, index) => (
                                  <div className="pl-6 rounded-md" key={index}>
                                    <span>
                                      {`Đáp án ${index + 1}: `}{" "}
                                      <span
          ...
          <div className="flex items-center gap-x-1 m-4">
            <h3 className="block font-bold ">Practice: </h3>
            <Link
